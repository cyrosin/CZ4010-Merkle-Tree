...
prover = Hasher()
prover2 = Hasher()
prover2.chunk_state = ChunkState(IV, 1, 0)

prover3 = Hasher()
prover3.chunk_state = ChunkState(IV, 2, 0)

prover3.update(message_3072)
prover3.update(message_4096)

prover.update(message_1024)
print(f"Prover Output H_1: {prover.chunk_state.output().root_output_bytes(length=32).hex()}")
print(f"Prover Output H_1 CV: {prover.chunk_state.output().chaining_value()}")
prover2.update(message_2048)
print(f"Prover 2 Output H_2: {prover2.chunk_state.output().root_output_bytes(length=32).hex()}")
print(f"Prover 2 Output H_2 CV: {prover2.chunk_state.output().chaining_value()}")
hash1 = prover.chunk_state.output()
hash2 = prover2.chunk_state.output()

hash1_2_cv = parent_cv(hash1.chaining_value(), hash2.chaining_value(), IV, PARENT)
...
print(f"Actual Output: {python_hasher.finalize().hex()}")
