...
impl Blake3Class {
    ...
    #[args(bytes)]
    fn get_words(&self, bytes: [u8; 32]) -> [u32; 8] {
        let words = blake3::platform::words_from_le_bytes_32(&bytes);
...
#[pyclass(name = "MiniHasher")]
#[pyo3(
    text_signature = "(data=b'', chunk_counter=0, /, *, key=None, derive_key_context=None, max_threads=1, usedforsecurity=True)"
)]
struct MiniHasherClass {
    // We release the GIL while updating this hasher, which means that other
    // threads could race to access it. Putting it in a Mutex keeps it safe.
    rust_chunk_state: blake3::guts::ChunkState
}

#[pymethods]
impl MiniHasherClass {
    #[new]
    #[args(
        chunk_counter,
        "/",
    ...
    fn new(
        chunk_counter: u64
    ) -> PyResult<MiniHasherClass> {
        let mini_hasher = blake3::guts::ChunkState::new(chunk_counter);

        Ok(MiniHasherClass { rust_chunk_state: (mini_hasher) })
    }

    #[args(input, "/")]
    #[pyo3(text_signature = "(input, /)")]
    fn update (&mut self, py: Python, input: &PyAny) -> PyResult<()> {
        let slice: &[u8] = unsafe { unsafe_slice_from_buffer(py, input)? };

        self.rust_chunk_state.update(slice);
        
        Ok(())
    }

    #[args()]
    fn get_chaining_values (&mut self) -> [u32; 8] {
        let cv: blake3::Hash = self.rust_chunk_state.finalize(false);
        let bytes = cv.as_bytes();
        let words = blake3::platform::words_from_le_bytes_32(bytes);
...
// #[pyfunction]
// #[pyo3(text_signature = "(left_child, right_child, /")]
// fn parent_cv(left_child: &[u32; 8], right_child: &[u32; 8]) -> [u32; 8] {
//     let left_bytes = blake3::platform::le_bytes_from_words_32(left_child);
//     let right_bytes = blake3::platform::le_bytes_from_words_32(right_child);

//     let &left_hash = blake3::Hash::from(left_bytes);
//     let &right_hash = blake3::Hash::from(right_bytes);

//     let parent = blake3::guts::parent_cv(left_hash, right_hash, false);

//     let parent_bytes = parent.as_bytes();
//     let words = blake3::platform::words_from_le_bytes_32(parent_bytes);
...
// #[pyfunction]
// #[pyo3(text_signature = "(left_child, right_child, /")]
// fn parent_output(left_child: &[u32; 8], right_child: &[u32; 8]) -> [u32; 8] {
//     let left_bytes = blake3::platform::le_bytes_from_words_32(left_child);
//     let right_bytes = blake3::platform::le_bytes_from_words_32(right_child);
...
//     let parent = blake3::guts::parent_cv(left_hash, right_hash, true);
